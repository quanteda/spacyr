...
The Penn Treebank is specific to English parts of speech.  For other language models, the detailed tagset will be based on a different scheme.  In the German language model, for instnace, the universal tagset (`pos`) remains the same, but the detailed tagset (`tag`) is based on the [TIGER Treebank](https://spacy.io/docs/usage/pos-tagging#pos-tagging-german) scheme.  Full details are available from the [spaCy models web page](https://spacy.io/models/).
...
```{r}
parsedtxt <- spacy_parse(txt, lemma = FALSE, entity = TRUE, nounphrases = TRUE)
...
The latter is useful for say, selecting only VERBS, using "glob" pattern matching with **quanteda**'s `tokens_select()` function:
```{r}
spacy_parse("The cat in the hat ate green ham and eggs", pos = TRUE) %>%
...
If you prefer a tidy approach to text analysis, **spacyr** works well with this approach because it returns parsed texts and (optionally) tokenized texts as data.frame-based objects. 
```{r}
